...
const runBuild = ({
  ...
  async function executeBuild() {
    ...
    /**
     * A declaration for the stylesheets this package publishes.
     *
     ...
     *
     * One file per package rather than one beside each stylesheet. Every CSS
     * subpath points its `types` condition here, and there is nothing to say in
     * any of them: a stylesheet is imported for its side effect and exports
     * nothing. `export {}` is what makes it a module rather than a script.
     */
    const emitCssDeclaration = () => {
      const hasCss = fs
        .readdirSync('dist', { recursive: true })
        .some(entry => String(entry).endsWith('.css'));
      if (!hasCss) return;
