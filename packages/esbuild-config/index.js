...
/**
 * A declaration beside every stylesheet in `dist`.
 *
 ...
 * and then every `import '…/button/style'` is an error: TypeScript resolves the
 * specifier, finds a `.css` file, and has nothing to say about it. The first
 * line of this system's getting-started is such an import.
 *
 * One per stylesheet rather than one per package, though they are identical. A
 * single shared declaration works for TypeScript and then tells every other
 * tool that the twenty-four subpaths are one module: `import/no-duplicates`
 * reported a file importing the same thing twenty-four times. The specifiers
 * differ, so the types they resolve to have to differ too.
 *
 * There is nothing to say in any of them. A stylesheet is imported for its side
 * effect and exports nothing; `export {}` is what makes the file a module
 * rather than a script.
 *
 * Exported because a package can write a stylesheet after esbuild has finished.
 * `style-tokens` generates two of them from its own scripts, and they went out
 * in 1.0.0 with `types` pointing at declarations nothing had written.
 */
...
module.exports.emitCssDeclarations = emitCssDeclarations;
